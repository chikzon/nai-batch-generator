...
- 스마트폰 앱·PC 연동, 자동 업데이트, 다국어 UI, 스트리밍 중간 미리보기는 없습니다.
- Vibe와 Character Reference는 NAI 전송 규칙 때문에 동시에 활성화할 수 없습니다.
  앱은 어느 자료도 삭제하지 않고 전송 전에 선택을 요구합니다.
- 팩 전체를 하나로 커밋하는 다중 파일 트랜잭션은 없습니다. 현재는 전체 충돌을 먼저
  검사하고 파일별 원자 저장과 판별 가능한 Undo를 사용합니다.
- 백업 복원은 현재값과 들어오는 값을 비교합니다. 모든 옛 백업에서 공통 기준본까지
  보여 주는 완전한 시각적 3-way 병합은 아직 없습니다.
- 로그인 세션이 필요한 공개자료 수집과 대형 archive의 Range 재개는 지원하지 않습니다.
...
- 자동 업데이트와 이전 설치 데이터 미리보기
- 공통 기준본을 포함한 시각적 3-way 충돌 해결
- 프로젝트·템플릿 계층 편집과 더 일반적인 실험 축
- 공개자료 로그인 세션과 대형 archive 재개
