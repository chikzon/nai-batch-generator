...
2. 열린 화면에서 `관리 → API`에 NovelAI Persistent API Token을 넣습니다.
...
아래 이미지는 v1.1.0의 현재 작업실 배치와 `종이` 테마로 직접 촬영했습니다.
테마와 개인 자료 상태에 따라 색상·항목 수는 달라질 수 있지만 기능 위치는 같습니다.

첫 실행에서는 빈 데이터 공간에서 필요한 순서를 화면 안에서 안내합니다.
...
화면 구성, 테마, 강조색, 글씨 크기와 모서리는 `관리 → 화면·디자인`에서 바꿀 수 있습니다.
...
| 작업 | 제공 기능 |
| --- | --- |
| 생성 | 프롬프트·캐릭터·생성 설정, 단건/다중 이미지 임포트, img2img·인페인트·바이브·레퍼런스·디렉터 툴·업스케일, 최근 결과 |
| 세팅 | 단계·옵션·캐스트, 순회/동시 출연, 캐릭터별 위치·네거티브·레퍼런스·바이브, 그림체·캐릭터·조합 비교 실험 |
| 자료 | 공개자료 수집, 자료팩 장착·독립 되돌리기, 통합 자료실, 출처·중복 장부, 별점·블라인드 ELO·월드컵·복구 휴지통 |
| 빌더 | 그림체·작가 조합·캐릭터·프롬프트 조각·태그, 그림체의 프롬프트+네거티브+생성 설정 묶음 저장 |
| 관리 | Queue·실행 기록·API·프로필·출력·백업·진단·화면 디자인 |
...
- NovelAI 규칙에 따라 Vibe와 캐릭터 레퍼런스는 동시에 활성화할 수 없습니다.
