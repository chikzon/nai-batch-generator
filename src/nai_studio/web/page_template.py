      ...
      <div class="card" id="libraryPublicCard" data-library-panel="input">
        ...
        <details id="relayPairingBox"
          ...
          <p class="hint" style="margin:6px 0;">
            아카라이브에 로그인된 브라우저에서 글을 연 상태로 아래 전달 코드를
            실행하면, 그 글의 본문과 이미지만 이 앱으로 넘어옵니다.
            받은 자료는 위의 수집 결과와 같은 곳에 정리됩니다.
            <b>연결 번호는 앱을 다시 켤 때마다 새로 발급</b>되며, 새로 발급하면
            이전 번호는 즉시 무효입니다.
          </p>
          <p class="hint" style="margin:6px 0;">
            <b>쓰는 법</b> — 아무 페이지나 북마크(즐겨찾기)로 저장한 뒤 그 북마크의
            주소를 아래 코드로 바꿔 두고, 아카라이브 글에서 그 북마크를 누르면
            됩니다. 주소창에 직접 붙여넣으면 브라우저가 보안상 맨 앞의
            <code>javascript:</code>를 지우므로, 그때는 그 여덟 글자와 콜론을
            손으로 다시 입력해야 합니다. 처음 실행하면 브라우저가
            <b>"로컬 네트워크(127.0.0.1) 연결을 허용할까요?"</b>라고 묻습니다 —
            허용해야 이 앱으로 전달됩니다(거부하면 "Failed to fetch"로 실패).
            CDN이 원본 전송을 막은 이미지는 건너뛰고 개수를 알려 줍니다.
          </p>
          <div class="bar" style="flex-wrap:wrap;">
...
<script src="/ui/studio-builder.js"></script>
<script src="/ui/studio-admin.js"></script>
<script src="/ui/studio-bootstrap.js"></script>
