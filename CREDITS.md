...
| 자료 | 출처 · 조건 |
|---|---|
| `t5_tokenizer.json` | NovelAI 가 쓰는 T5 토크나이저 vocab. 토큰 수 계산에만 쓴다. 공개 자료에서 취득했으나 **정확한 원 출처를 기록해 두지 못했다** — 확인되는 대로 갱신한다 |
| `태그/*.csv` | 단부루 · e621 태그 목록(이름 + 게시물 수). 공개 자료에서 취득했다(다른 NAIS 프로젝트에서 가져온 것이 아니다). 정확한 취득처는 기록해 두지 못했다 — 원 출처의 조건을 각자 확인할 것 |
| `수집/그림체.json` · `작가통계.json` · `레시피.json` | 아카라이브 · 도랑 위키에서 사람들이 공개한 프롬프트 조합을 모은 것. 본 배포본 대신 **자료팩.zip 으로 따로** 전달한다 |
| `수집/이미지캐시/` | 위 그림체의 예시 이미지. **남이 만든 그림이라 저장소·배포본에 넣지 않는다** (자료팩으로만) |
