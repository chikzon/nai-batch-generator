...
기준: `4a23fc4` 이후 작업 트리. 이 문서는 완료 선언이 아니라 누락 방지 색인이다.

## 전수 범위

| 대상 | 실측 | 분류 상태 |
|---|---:|---|
| 제품 Python | 74파일 · 1,095개 함수·클래스 | 디렉터리·모듈 소유권으로 전부 배정 |
| 50줄 이상 제품 정의 | 151개 | 별도 소유 모듈 73개, `legacy_app.py` 잔존 78개 |
...
78개 잔존 정의와 `studio.js` 분리가 남아 있으므로 구조 최적화는 **진행 중**이다.
