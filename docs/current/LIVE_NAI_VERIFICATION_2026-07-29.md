...
이 문서는 실제 NAI 호출과 당시 화면 계측의 증거다. 현재 구현·남은 작업·전체 회귀
상태는 `../../../ai-review/01-기능증거색인.md`를 우선한다.
...
- 전체 백업의 충돌을 파일·JSON 항목별 현재값/들어오는 값으로 확인하고, 고른
...
- 스마트폰 앱·PC 연동
- 팩 전체를 하나의 journal로 묶는 다중 파일 트랜잭션. 현재는 manifest와 모든
  충돌을 먼저 검사하고 파일별 원자 저장·판별 Undo를 사용
- 실제 공통 기준본까지 보이는 완전한 3-way 병합. 현재 백업에 기준 해시가 있으면
  표시하지만, 기존 백업은 현재값/들어오는 값의 2-way 선택 복원
- 캐릭터 이미지 Bench·Reference inset의 실제 유료 NAI 호출. 요청 사본, canvas/mask,
  대표·근거·variation 저장과 최종 payload 연결은 `ec7df63`에서 구현·무과금 검증됐다.
- 범용 조건 규칙/DSL과 완전한 X/Y 실험 라벨 그리드는 채택 범위가 아니다.
